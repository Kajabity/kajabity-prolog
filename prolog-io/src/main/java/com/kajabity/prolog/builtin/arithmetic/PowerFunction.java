 ...
 * /home/cvs/cvs/java-prolog/src/engine/uk/co/williams_technologies/prolog/builtin/arithmetic/PowerFunction.java,v
 * 1.3 2004/05/05 05:28:38 simon Exp $
 * ******************************************************************************
 * $Log: PowerFunction.java,v $ Revision 1.3 2004/05/05 05:28:38 simon Lots of
...
{
    public PowerFunction( String name, int arity )
    {
        super( name, arity );
    }
    ...
    {
        NumericConstant right = values.pop();
        NumericConstant left = values.pop();

        if( right.isReal() || left.isReal() )
            return new NumericConstant( Math.pow( left.doubleValue(), right
                    .doubleValue() ) );
        else
