 ...
 */
public class AssertGroundLiteralPredicate extends GroundLiteral
{

    public AssertGroundLiteralPredicate( Database database, String name, int arity )
    {
    ...
    protected List<Variable> execute( Goal goal ) throws TokenException, IOException, PrologException
    {
        ...
        if( goal.getTerm() instanceof Tuple )
        {
            ...
            {
                List<Expression> args = params.getArgs().getTerms();
                ...
                {
                    List<Expression> specArgs = ((Tuple) p1).getArgs().getTerms();

                    Expression p3 = specArgs.get( 0 );
                    Expression p4 = specArgs.get( 1 );
                    ...
                    if( p3 instanceof Atom && p4 instanceof NumericConstant && p2 instanceof Atom )
                    {
                        try
                        {
                            ...
                            Constructor<?> predicateConstructor = predicateClass.getConstructor( Database.class, String.class,
                                    int.class );

                            GroundLiteral groundLiteral = (GroundLiteral) predicateConstructor.newInstance( database, ((Atom) p3)
