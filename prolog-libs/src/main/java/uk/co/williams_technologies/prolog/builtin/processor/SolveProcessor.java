...
public class SolveProcessor extends Processor
{
     ...
     *
     * @param name
     *            the Tuple name used by this processor (i.e. "?-").
     */
    ...
    public void execute( Database db, Term term ) throws TokenException, IOException, PrologException
    {
        ...
        if( args.size() != 1 )
        {
            throw new PrologException( "Solve: Expected a single arg (Term or TermList) : " + term );
        }
                ...
                for( Variable v : variables.values() )
                {
                    if( !v.isAnonymous() )
                    {
                        db.getCurrentOutputStream()
                                .println( "\t" + v.getName() + " = " + format.format( v.getFinalInstantiation() ) );
                    }
                }

                if( wantMore( db.getCurrentOutputStream(), db.getCurrentInputStream() ) )
                {
                    // Try again..
                    solved = false;
                }
    ...
    private boolean wantMore( PrintStream out, InputStream in ) throws IOException
    {
        out.println( "more? " );
